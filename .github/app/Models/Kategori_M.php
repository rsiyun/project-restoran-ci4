<?php

namespace App\Models;

use CodeIgniter\Model;

class Kategori_M extends Model
{
    protected $table = 'tblkategori';
    protected $allowedFields = ['kategori', 'keterangan'];
    protected $primaryKey = 'idkategori';

    protected $validationRules  = [
        'kategori' => 'alpha_numeric_space|min_length[3]|is_unique[tblkategori.kategori]',
    ];
    protected $validationMessages = [
        'kategori' => [
            'alpha_numeric_space' => 'kategori cannot contain symbols',
            'min_length' => 'sorry minimum kategori 3 letter numbers',
            'is_unique' => 'sorry kategori has been taken please choose another'
        ]
    ];
}
